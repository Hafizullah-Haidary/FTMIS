...
body {
    ...
    <body>
        {{-- <h1>Search Results for "{{ $emp_id }}"</h1> --}}
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ route('foreigntrip.destroy', $foreigntrip->id) }}" method="POST">
                            ...
                            <a class="btn btn-info" href="{{ route('foreigntrip.show', $foreigntrip->id) }}">{{ trans('foreigntrip.show') }}</a>

                            <a class="btn btn-primary" href="{{ route('foreigntrip.edit', $foreigntrip->id) }}">{{ trans('foreigntrip.edit') }}</a>

                            @csrf
                            @method('DELETE')

                            <button type="submit" class="btn btn-danger">{{ trans('foreigntrip.delete') }}</button>
                        </form>
                    </td>
                </tr>
            @endforeach
